...
#
# Script generated by Vivado on Wed Jan 07 21:39:59 IST 2026
...
#
# Usage        : V.sh
#                V.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                V.sh [-reset_run]
#                V.sh [-reset_log]
#                V.sh [-help]
#
...
#                script (type 'V.sh -help' for more information).
#
...
# script info
echo -e "V.sh - Script generated by export_simulation (Vivado v2023.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./V.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.V xil_defaultlib.glbl -o V_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./V_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key V_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc V_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./V.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: V.sh [-help]\n\
Usage: V.sh [-step]\n\
Usage: V.sh [-lib_map_path]\n\
Usage: V.sh [-reset_run]\n\
Usage: V.sh [-reset_log]\n\
Usage: V.sh [-keep_index]\n\
Usage: V.sh [-noclean_files]\n\n\
