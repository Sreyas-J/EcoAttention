...
#
# Script generated by Vivado on Fri Aug 29 18:41:53 IST 2025
...
#
# Usage        : CACHE.sh
#                CACHE.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                CACHE.sh [-reset_run]
#                CACHE.sh [-reset_log]
#                CACHE.sh [-help]
#
...
#                script (type 'CACHE.sh -help' for more information).
#
...
# script info
echo -e "CACHE.sh - Script generated by export_simulation (Vivado v2023.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./CACHE.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.CACHE xil_defaultlib.glbl -o CACHE_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./CACHE_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key CACHE_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc CACHE_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./CACHE.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: CACHE.sh [-help]\n\
Usage: CACHE.sh [-step]\n\
Usage: CACHE.sh [-lib_map_path]\n\
Usage: CACHE.sh [-reset_run]\n\
Usage: CACHE.sh [-reset_log]\n\
Usage: CACHE.sh [-keep_index]\n\
Usage: CACHE.sh [-noclean_files]\n\n\
