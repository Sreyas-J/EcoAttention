...
#
# Script generated by Vivado on Mon Aug 11 23:04:09 IST 2025
...
#
# Usage        : O.sh
#                O.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                O.sh [-reset_run]
#                O.sh [-reset_log]
#                O.sh [-help]
#
...
#                script (type 'O.sh -help' for more information).
#
...
# script info
echo -e "O.sh - Script generated by export_simulation (Vivado v2023.1 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./O.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.O xil_defaultlib.glbl -o O_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./O_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key O_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc O_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./O.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: O.sh [-help]\n\
Usage: O.sh [-step]\n\
Usage: O.sh [-lib_map_path]\n\
Usage: O.sh [-reset_run]\n\
Usage: O.sh [-reset_log]\n\
Usage: O.sh [-keep_index]\n\
Usage: O.sh [-noclean_files]\n\n\
